		...
		<div class="row">
			<div class="col-lg-12 col-md-12">
				<ol class="breadcrumb">
					...
					<li class="active">
						<strong>Edit Production Planning</strong>
					</li>					
				</ol>
			</div>
		</div>
	...
	<div class="pageHeading container-fluid">
		<div class="row ">			
			<div class="page-title-wrapper">			
				<div class="col-lg-12">
					<h2 class="page-header">Edit Production Planning</h2>					
				</div>							
			</div>			
		</div>
	</div><!-- pageHeading end -->
		...
		<div class="row">
			<div class="col-xs-12">
				<section id="inputDetail" class="form-container">				
                    {!! Form::model($plan, array('method' => 'post', 'url' => 'planning/update/'.$plan->id,'name'=>'frm','onsubmit'=>' return validatePlanning()','id'=>'addPlan')) !!}
						<div class="form-control-wrapper">
							<div class="form-group">
								<div class="row">
									<div class="col-sm-4 ddl"> 						
										{!! HTML::decode( Form::label('machine', 'Machine <small class="mandatory">*</small>')) !!}
										{!! Form::select('machine_id',$data['machine'],null,['id'=>'machine_id','class'=>'form-control','onchange'=>'getPlanning(this.value)','disabled'=>true]) !!}
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-sm-4"> 						
										{!! HTML::decode( Form::label('start_date', 'Start Date Time <small class="mandatory">*</small>')) !!}
                                        {!! Form::text('start_date_time',null,array('placeholder'=>'Start Date Time','id'=>'start_date_time','class'=>'form-control','readonly'=>true,'onblur'=>'validatePlanningDatetime(this.value)','disabled'=>true)) !!}
							...
							<div class="form-group">
								<div class="row">
									<div class="col-sm-4"> 						
										...
										{!! Form::text('planning_quantity',null,array('placeholder'=>'Planning Quatity','id'=>'planning_quantity','class'=>'form-control','maxlength'=> 11,'autocomplete'=>'off')) !!}
									...
									<div class="col-sm-4"> 						
										...
										{!! Form::select('status',$data['status'],'A',['id'=>'status','class'=>'form-control','onchange'=>'checkStatus(this.value)']) !!}
										<div id="status_alert" class="error validationAlert validationError">{!!$errors->first('status')!!}</div>
									</div>
									<div class="col-sm-4" id="closeDatetimeDiv" style="display:none">
										{!! HTML::decode(Form::label('close_date', 'Close Date Time <small class="mandatory">*</small>')) !!}
                                        {!! Form::text('close_date_time',date("d-m-Y H:i"),array('placeholder'=>'Close Date Time','id'=>'close_date_time','class'=>'form-control','readonly'=>true)) !!}
										<div id="close_date_time_alert" class="error validationAlert validationError">{!!$errors->first('close_date_time')!!}</div>
									</div>
								</div>
							</div>	
...
<script type="text/javascript">
    ...
    function validatePlanning()
    {
        ...
        if($("#status").val() == 'SC' || $("#status").val() == 'C'){
        	arrElemId[i]    = 'close_date_time';
	        arrCode[i]      = 'IS_EMPTY';
	        arrRefValue[i]  = null;
	        arrMsg[i]       = '<?php echo Lang::get("messages.close_date_time")?>';
	        i++;
        }
    ...
    function getProducts(moldId,selected){
    	if(moldId != ''){
    ...
    $(document).ready(function () {
    	getProducts('<?php echo $plan->mold_id?>','<?php echo $plan->product_id?>');
    	var status = $("#status").val();
    	if(status == 'C' || status == 'SC'){
			$("#closeDatetimeDiv").show();
    	}
	});
    
  //  $('#start_date_time').bind('focus', function () {
  	...
  	/*	theme:'dark',*/
   	...
   	<?php if(Lib::isAdmin()){ ?>
	  	...
	  	/*	theme:'dark',*/
	  		...
	  		onSelectTime : function(dp,$input){
	  			$("#close_date_time").datetimepicker('hide');
	  			$input.blur();
	  			//$("#planning_quantity").focus();
	  		}
	    }); 
    	...
    	if(validateDatetime(datetime)){
    		if(validateYesterdaysDate(datetime)){
    			...
    			$.ajax({
	                ...
	                method : "POST",
	                data : { 'date' : datetime, 'machineId' : machineId , 'planId' : '<?php echo $plan->id?>'},
	                cache : false,
	                beforeSend : function(){
	                    //$("#loading").show();
	                },
