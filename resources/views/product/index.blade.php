...
<div id="main-container" class="main-container">
	...
	<div class="searchForm " style="display:none;">
		...
		{!! Form::open(array('route' => 'product.index','method'=>'GET','onSubmit' => 'return validateSearch()')) !!}
		<div class="form-control-wrapper">
			<div class="form-group">
				<div class="row">
					<div class="col-sm-4"> 						
						{!! Form::label('Shape', 'Trade Name') !!}
						{!! Form::text('tradeName',Request::get('tradeName'),array("placeholder"=>"Trade Name",'id'=>'tradeName','class'=>'form-control')) !!} 
						<div id="trade_name_alert" class="error validationAlert validationError"></div>
					</div>					
					<div class="col-sm-4">											
			...
			<div class="form-group action">
				<div class="col-lg-12 pull-center buttons"> 
					...
					<button type="button" class="btn btn-black" onclick="doCancel('{!! URL::to('product') !!}')">Cancel</button>
				</div>									
			</div><!-- action end -->
	...
	<div class="pageHeading container-fluid">
		<div class="row ">			
			<div class="page-title-wrapper">			
				<div class="col-lg-8">
					<h2 class="page-header">Manage Products</h2>					
				</div>
				<div class="col-lg-4">
					<div class="title-action">
						@if(count($products))
						<a title="Download Sheet" class="btn btn-white download-sheet" href="{!! URL::to('product/export') !!}">
							<i class="fa fa-file-excel-o"></i> Export
						</a>
						@endif
						<a title="Add Product" class="btn btn-white add-shape add-icons" href="{!! route('product.create')!!}">
			...
			<div class="col-xs-12">
				...
				<div class="table-responsive">
					<table id="drawingList" class="category table table-hover">
					  ...
					  <?php $count = ($products->currentPage() * Config::get('global_vars.PAGINATION_LIMIT')) - Config::get('global_vars.PAGINATION_LIMIT') + 1;
					  	?>
						<tbody>
							@foreach($products as $key)
							<tr>
								<td align="right">{!! $count++ !!}</td>
								<td>{!! $key->trade_name !!}</td>
								<td>{!! $data['drawing'][$key->drawing_id]!!}</td>
								<td>{!! $data['color'][$key->color_id]!!}</td>
								<td>{!! $data['brand'][$key->brand_id]!!}</td>
								<td>{!! $data['primaryApplication'][$key->primary_application_id]!!}</td>
								...
								<td align="center" class="action-icon">								
									<a href="{!! URL::to('product/'.$key->id.'/edit') !!}" data-toggle="tooltip" data-placement="bottom" title="Edit Product" class="space">
										<i class="glyphicon glyphicon-pencil"></i> 
									</a>
								{!! Form::open(array('id'=>'destroyProduct'.$key->id,'method' => 'DELETE', 'class'=>'delete-icon','route' => array('product.destroy', $key->id))) !!}
...
function confirmDelete(id) {
    jConfirm('<?php echo Lang::get('messages.CONFIRM_DELETE'); ?>'+" this product?", '', function(r) {
        if (r) {
    		$('#destroyProduct'+id).submit();
        }
    });
}
</script>
