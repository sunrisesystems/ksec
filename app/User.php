...
class User extends Model implements RoleableInterface, PermissibleInterface, PersistableInterface, UserInterface
{
     ...
     * @var array
     */
    protected $loginNames = ['username','unit_id','status'];
    ...
    public function getUsers($input)
    {
        ...
        if(isset($input['name']) && !empty($input['name'])){
            $query = $query->where('first_name','like','%'.$input['name'].'%')->orWhere('last_name','like','%'.$input['name'].'%');
        }
        if(isset($input['unit']) && !empty($input['unit'])){
            $query = $query->where('unit_id',$input['unit']);
        }
